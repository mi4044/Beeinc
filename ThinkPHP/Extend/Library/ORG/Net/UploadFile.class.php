...
class UploadFile {//类定义开始
    ...
    private $config =   array(
        ...
        'dateFormat'        =>  'Ymd',//当使用日期date 作为子目录时，目录格式
    ...
    private function save($file) {
        ...
        if( in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png','swf'))) {
            //todo 这为了兼容 场景，产品，铺贴模板，案列等，的图片加密无法识别问题
            $module=array('Scene','House','Schemepaves','Paves','Product');
            if (!(strtolower(ACTION_NAME) == 'uploadfile' && in_array(MODULE_NAME, $module))) {
     ...
     * @param string $suffix 上传文件保存名称 @todo wangzhoubin@live.com 2014/4/27修改 添加$fileName 可以自己设置文件名
     * @return string
     */
    public function upload($savePath ='',$suffix='') {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                $file['savename']   =   $this->getSaveName($file,$suffix);//@todo wangzhoubin@live.com 2014/4/27修改 添加$fileName 可以自己设置文件名
        ...
        if($isUpload) {
            ...
            //todo 上传成功 存入数据库
            //wangzhoubin@live.com  7/28/2016
            $fileInfo=array_map(function ($value){
                return $value['savepath'].$value['savename'];
            },$fileInfo);
            file_upload_log($fileInfo);
            return true;
        }else {
            $this->error  =  '没有选择上传文件';
            return false;
     ...
     * @return string
     */
    public function uploadnews($savePath ='') {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
            $savePath = $this->savePath;
        // 检查上传目录
     ...
     * @param string $suffix 自定义文件名后缀@todo wangzhoubin@live.com 修改2016/4/27
     * @return string
     */
    private function getSaveName($filename,$suffix='') {
        $rule = $this->saveRule;
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            $saveName = $filename['name'].$suffix;
        }else {
            if(function_exists($rule)) {
                //使用函数生成一个唯一文件标识号
                $saveName = $rule().$suffix.".".$filename['extension'];
            }else {
                //使用给定的文件名作为标识号
                $saveName = $rule.$suffix.".".$filename['extension'];
            }
