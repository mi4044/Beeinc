...
<nav class="navbar navbar-custom navbar-fixed-top " style="border-width: 0 0 30px" role="navigation" id="nav">
        ...
        <div id='scroll-1'>    
            <ul class="left-class">
                <?php if(is_array($cate)): $k = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><li>
					<?php if($vo['type'] == '下载安装注册' ): ?><a class="tab-active" data-index="<?php echo ($k-1); ?>" href="#">— <?php echo ($vo['type']); ?></a>
					<?php else: ?>
					<a data-index="<?php echo ($k-1); ?>" href="#">— <?php echo ($vo['type']); ?></a><?php endif; ?>
				</li><?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
        </div>>
        <div class="tabs-content-placeholder">

            <?php if(is_array($mfrj)): $i = 0; $__LIST__ = $mfrj;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v['type'] == '下载安装注册' ): ?><div class="tab-content-active">
...
<script>
    ...
    $(document).on('click','ul li a',function(){
        if(index2!=$(this).attr("data-index")){
            ...
            $('html,body').stop().animate({scrollTop:_scrollTop},350);
